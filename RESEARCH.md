...
**Campus itemisation.** With Telehouse North added, the **Telehouse Docklands** aggregate is now fully decomposed into its halls (North, North Two, West, West Two, South, East); the **Equinix Slough** aggregate is likewise complete against the operator roster (LD4/5/6/7/10/11x/13x/14), as is **Kao Harlow** (KLON-01–04). The aggregates are **kept, not retired**: they are already excluded from the map and from every total / table / compare (via `getItemizedSites`, so no double-count), and by design they remain as campus index pages that each hall links back to via `part_of` (see `types.ts`). Deleting them would only remove those campus pages and the hall cross-links for no functional gain. Notes: `telehouse-south` (1 Blackwall Way) is confirmed real — Telehouse's fifth and largest London DC, opened 2022 (the earlier four-building brochure predates it); **Telehouse Central** (2026 administration building) is deliberately not mapped as it is office/admin, not a data hall.
